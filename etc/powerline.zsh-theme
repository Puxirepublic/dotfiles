...
# else
    if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            POWERLINE_GIT_INFO_LEFT=" %F{magenta}%K{black}"$'\ue0b0'"%F{white}%F{white}%K{black}"$'$(git_prompt_info)$(git_prompt_status)%F{black}'
        else
            POWERLINE_GIT_INFO_LEFT=" %F{magenta}%K{black}"$'\ue0b0'"%F{white}%F{white}%K{black}"$'$(git_prompt_info)%F{black}'
        fi
        POWERLINE_GIT_INFO_RIGHT=""
    else
        POWERLINE_GIT_INFO_LEFT=""
        POWERLINE_GIT_INFO_RIGHT="%F{black}"$'\ue0b2'"%F{white}%K{black}"$'$(git_prompt_info)'" %K{black}"
    fi
# fi
...
PROMPT="$POWERLINE_SEC1_BG$POWERLINE_SEC1_TXT $POWERLINE_USER_NAME %k%f$POWERLINE_SEC1_FG%K{magenta}"$'\ue0b0'"%k%f%F{black}%K{magenta} "$POWERLINE_CURRENT_PATH"%F{magenta}"$POWERLINE_GIT_INFO_LEFT" %k"$'\ue0b0'"%f "
...
if [ "$POWERLINE_DISABLE_RPROMPT" = "" ]; then
    if [ "$POWERLINE_RIGHT_A" = "" ]; then
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{black}"$'\ue0b2'"%k%F{white}%K{black} $POWERLINE_RIGHT_B %f%k"
    elif [ "$POWERLINE_RIGHT_B" = "" ]; then
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{black}"$'\ue0b2'"%k%F{240}%K{black} $POWERLINE_RIGHT_A %f%k"
    else
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{black}"$'\ue0b2'"%k%F{white}%K{black} $POWERLINE_RIGHT_B %f%F{240}"$'\ue0b2'"%f%k%K{240}%F{255} $POWERLINE_RIGHT_A %f%k"
    fi
fi
